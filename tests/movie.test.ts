...
describe('Movie Parser', () => {
  it(
    'should get detailed information for a movie and save to a file',
    async () => {
      ...
      if (!login || !password) {
        console.warn(
          'Skipping "movie" test: TEST_LOGIN and TEST_PASSWORD environment variables are not set.'
        );
        return;
      }
      ...
      if (movieDetails.currentWatch && movieDetails.seasons) {
        const { streams, subtitles } = await scraper.stream.get(
          movieDetails.id,
          movieDetails.currentWatch.translatorId,
          movieDetails.currentWatch.season,
          movieDetails.currentWatch.episode
        );
        const seasonIndex = movieDetails.currentWatch.season - 1;
        const episodeIndex = movieDetails.currentWatch.episode - 1;

        if (
          movieDetails.seasons[seasonIndex] &&
          movieDetails.seasons[seasonIndex].episodes[episodeIndex]
        ) {
